...
from nis_mc.integrators.nis_integrator import NISIntegratorNumPy
...
def test_nis_accuracy(n_eval):
    ...
    bw = BreitWigner1D()
    integrator = NISIntegratorNumPy(n_train_samples=1000)
...
def test_benchmark_dataframe_columns():
    ...
    n_integrator = NISIntegratorNumPy(n_train_samples=100)
...
def test_reproducibility():
    ...
    # NIS
    n1 = NISIntegratorNumPy(n_train_samples=100)
    np.random.seed(42)
    res_n1 = n1.integrate(bw, bounds, n_eval=2000)
    
    n2 = NISIntegratorNumPy(n_train_samples=100)
    np.random.seed(42)
